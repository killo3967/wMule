...
public:
	...
	///   HKDF is unusual in that a non-nullptr salt with length 0 is different than a
	///   nullptr <tt>salt</tt>. A nullptr <tt>salt</tt> causes HKDF to use a string of 0's
...
protected:
	...
	// If salt is absent (nullptr), then use the nullptr vector. Missing is different than
	// EMPTY (Non-nullptr, 0 length). The length of s_NullVector used depends on the Hash
    ...
    const byte *salt, size_t saltLen, const byte* info, size_t infoLen) const
{
	...
	// HKDF business logic. nullptr is different than empty.
	if (salt == NULLPTR)
	{
		salt = GetNullVector();
