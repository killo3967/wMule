...
{
	// Avoid assert on pointer in reallocate. SecBlock regularly uses nullptr
	// pointers rather returning non-nullptr 0-sized pointers.
	if (oldSize == newSize)
		return oldPtr;
...
public:
	...
	///  function asserts in Debug builds if ptr is non-nullptr.
	...
	{
		// Avoid assert on pointer in deallocate. SecBlock regularly uses nullptr
		// pointers rather returning non-nullptr 0-sized pointers.
		if (ptr)
		{
...
public:
	...
	/// \throw std::bad_alloc
	/// \details If <tt>ptr!=nullptr</tt> and <tt>len!=0</tt>, then the block is initialized from the pointer
	///  <tt>ptr</tt>. If <tt>ptr==nullptr</tt> and <tt>len!=0</tt>, then the block is initialized to 0.
