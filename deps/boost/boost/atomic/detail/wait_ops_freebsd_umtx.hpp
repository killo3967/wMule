    ...
    public Base
{
    ...
    {
        ::_umtx_op(const_cast< storage_type* >(&storage), UMTX_OP_WAKE, 1u, nullptr, nullptr);
    }

    static BOOST_FORCEINLINE void notify_all(storage_type volatile& storage) BOOST_NOEXCEPT
    {
        ::_umtx_op(const_cast< storage_type* >(&storage), UMTX_OP_WAKE, (~static_cast< unsigned int >(0u)) >> 1, nullptr, nullptr);
    }
};
    ...
    public wait_operations_freebsd_umtx_common< Base >
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) BOOST_NOEXCEPT
    {
        ...
        {
            ::_umtx_op(const_cast< storage_type* >(&storage), UMTX_OP_WAIT_UINT, old_val, nullptr, nullptr);
    ...
    public wait_operations_freebsd_umtx_common< Base >
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) BOOST_NOEXCEPT
    {
        ...
        while (new_val == old_val)
        {
            ::_umtx_op(const_cast< storage_type* >(&storage), UMTX_OP_WAIT, old_val, nullptr, nullptr);
            new_val = base_type::load(storage, order);
        }
