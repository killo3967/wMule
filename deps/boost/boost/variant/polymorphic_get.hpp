    ...
    ) BOOST_NOEXCEPT
{
    BOOST_STATIC_ASSERT_MSG(
        ...
        "call to boost::polymorphic_get<U>(boost::variant<T...>*) will always return nullptr"
    );
    ...
    ) BOOST_NOEXCEPT
{
    BOOST_STATIC_ASSERT_MSG(
        (boost::detail::variant::holds_element_polymorphic<boost::variant< BOOST_VARIANT_ENUM_PARAMS(T) >, U >::value),
        "boost::variant does not contain specified type U, "
        "call to boost::polymorphic_get<U>(const boost::variant<T...>*) will always return nullptr"
    );
