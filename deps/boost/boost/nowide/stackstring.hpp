...
namespace nowide {
    ...
    ///
    /// If a nullptr pointer is passed to the constructor or convert method, nullptr will be returned by c_str.
    /// Similarly a default constructed stackstring will return nullptr on calling c_str.
    ///
    ...
    public:
        ...
        /// Creates a nullptr stackstring
        basic_stackstring()
        {
            buffer_[0] = 0;
        }
        /// Convert the nullptr terminated string input and store in internal buffer
        /// If input is nullptr, nothing will be stored
        explicit basic_stackstring(const input_char* input)
        {
            convert(input);
        }
        /// Convert the sequence [begin, end) and store in internal buffer
        /// If begin is nullptr, nothing will be stored
        ...
        /// Convert the nullptr terminated string input and store in internal buffer
        /// If input is nullptr, the current buffer will be reset to nullptr
        ...
        /// Convert the sequence [begin, end) and store in internal buffer
        /// If begin is nullptr, the current buffer will be reset to nullptr
        output_char* convert(const input_char* begin, const input_char* end)
        {
            ...
            if(begin)
            {
                ...
                // Minimum size required: 1 output char per input char + trailing nullptr
                ...
                else
                {
                    ...
                    // Max size: Every input char is transcoded to the output char with maximum with + trailing nullptr
        ...
        /// Return the converted, nullptr-terminated string or nullptr if no string was converted
        output_char* get()
        {
            return data_;
        }
        ...
        /// Reset the internal buffer to nullptr
        void clear()
        {
    ...
    protected:
        ...
        /// Return the current length of the string excluding the nullptr terminator
        /// If nullptr is stored returns nullptr
        size_t length() const
        {
            if(!data_)
                return 0;
