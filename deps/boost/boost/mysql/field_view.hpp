...
public:
    /**
     * \brief Constructs a `field_view` holding nullptr.
     ...
     * \brief Constructs a `field_view` holding nullptr.
     * \details
     * Caution: `field_view(nullptr)` will <b>not</b> match this overload. It will try to construct
     * a `string_view` from a nullptr C string, causing undefined behavior.
     *
     * \par Exception safety
     * No-throw guarantee.
     *
    ...
    /**
     * \brief Returns whether this `field_view` points to a `nullptr` value.
     * \par Exception safety
     * No-throw guarantee.
     */
     ...
     * different between them). `nullptr` values are equal to other `nullptr` values.
     *
     * \par Exception safety
     * No-throw guarantee.
     */
