...
class voronoi_cell {
 public:
  ...
  bool is_degenerate() const { return incident_edge_ == nullptr; }
  ...
  void _build() {
    ...
    if (vertices_.empty()) {
      ...
      for (cell_iterator cell_it = cells_.begin();
        ...
        if (left_edge->prev() != nullptr)
          continue;

        edge_type* right_edge = cell_it->incident_edge();
        while (right_edge->next() != nullptr)
