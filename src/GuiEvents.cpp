...
namespace MuleNotify
{
	...
	{
#ifndef AMULE_DAEMON
		CastByID(IDC_NODESLISTURL, nullptr, wxTextCtrl)->SetValue(url);
#endif
	}
	...
	{
#ifndef AMULE_DAEMON
		CastByID(IDC_SERVERLISTURL, nullptr, wxTextCtrl)->SetValue(url);
#endif
	}
	...
	void PartFile_Swap_A4AF_Others(CPartFile* file)
	{
		if ((file->GetStatus(false) == PS_READY) || (file->GetStatus(false) == PS_EMPTY)) {
			...
			for( ; it != file->GetSourceList().end(); ) {
				it++->SwapToAnotherFile(false, false, false, nullptr);
			}
		}
	}
