...
//-------------------------------------------------------------------
IMPLEMENT_APP_CONSOLE (CamulecmdApp)
//-------------------------------------------------------------------
				...
				if (ipfilterLevel) {
					if (m_last_cmd_id == CMD_ID_GET_IPFILTER ||
						...
						s += CFormat(_("IP filtering for clients is %s.\n"))
								% ((tab->GetTagByName(EC_TAG_IPFILTER_CLIENTS) == nullptr) ? _("OFF") : _("ON"));
					}
					if (m_last_cmd_id == CMD_ID_GET_IPFILTER ||
					    m_last_cmd_id == CMD_ID_GET_IPFILTER_STATE ||
					    m_last_cmd_id == CMD_ID_GET_IPFILTER_STATE_SERVERS) {
						s += CFormat(_("IP filtering for servers is %s.\n"))
								% ((tab->GetTagByName(EC_TAG_IPFILTER_SERVERS) == nullptr) ? _("OFF") : _("ON"));
					}
					if (m_last_cmd_id == CMD_ID_GET_IPFILTER ||
		...
		case EC_OP_STATS: {
			...
			if (connState) {
				...
				if (connState->IsConnectedED2K()) {
					...
					const CECTag *serverName = server ? server->GetTagByName(EC_TAG_SERVER_NAME) : nullptr;
