			...
			if (	!getsizeonly ||
				(getsizeonly &&
					...
					case FT_KADLASTPUBLISHSRC:{
						...
						if(GetLastPublishTimeKadSrc() > (uint32)time(nullptr)+KADEMLIAREPUBLISHTIMES) {
			...
			switch (cur_src->GetDownloadState()) {
				...
				case DS_NONEEDEDPARTS: {
					...
					if((dwCurTick - lastpurgetime) > 40000) {
						if(!cur_src->SwapToAnotherFile(false , false, false , nullptr)) {
...
void CPartFile::UpdatePartsInfo()
{
	...
	bool flag = (time(nullptr) - m_nCompleteSourcesTime > 0);
	...
	if ( flag ) {
		...
		m_nCompleteSourcesTime = time(nullptr) + (60);
	}
	UpdateDisplayedInfo();
}
	...
	while(newBlockCount != count) {
		...
		if(sender->GetLastPartAsked() == 0xffff) {
			...
			if(chunksList.empty()) {
				...
				for(uint16 i=0; i < partCount; ++i) {
					if(sender->IsPartAvailable(i) == true && GetNextEmptyBlockInPart(i, nullptr) == true) {
...
void  CPartFile::RemoveAllSources(bool bTryToSwap)
{
	for( SourceSet::iterator it = m_SrcList.begin(); it != m_SrcList.end();) {
		CUpDownClient* cur_src = it++->GetClient();
		if (bTryToSwap) {
			if (!cur_src->SwapToAnotherFile(true, true, true, nullptr)) {
...
bool CPartFile::HashSinglePart(uint16 partnumber)
{
	if ((GetHashCount() <= partnumber) && (GetPartCount() > 1)) {
		...
		try {
			CreateHashFromFile(m_hpartfile, offset, length, &hashresult, nullptr);
		} catch (const CIOFailureException& e) {
...
void CPartFile::StopPausedFile()
{
	if (!IsStopped()) {
		...
		switch (GetStatus()) {
			...
			case PS_ERROR:
				if (time(nullptr) - m_iLastPausePurge > (60*60)) {
					m_iLastPausePurge = time(nullptr);
					StopFile();
				}
				kBpsDown = 0.0;
		}
...
void CPartFile::AddClientSources(CMemFile* sources, unsigned nSourceFrom, uint8 uClientSXVersion, bool bSourceExchange2, const CUpDownClient* /*pClient*/)
{
	...
	for (uint16 i = 0;i != nCount;++i) {
		...
		if (!CanAddSource(dwID, nPort, dwServerIP, nServerPort, nullptr, false)) {
...
void CPartFile::RequestAICHRecovery(uint16 nPart)
{
	...
	for ( SourceSet::iterator it = m_SrcList.begin(); it != m_SrcList.end(); ++it) {
		...
		if (	pCurClient->IsSupportingAICH() &&
			pCurClient->GetReqFileAICHHash() != nullptr &&
	...
	CUpDownClient* pClient = nullptr;
	for ( SourceSet::iterator it = m_SrcList.begin(); it != m_SrcList.end(); ++it) {
		CUpDownClient* pCurClient = it->GetClient();
		if (pCurClient->IsSupportingAICH() && pCurClient->GetReqFileAICHHash() != nullptr && !pCurClient->IsAICHReqPending()
			&& (*pCurClient->GetReqFileAICHHash()) == m_pAICHHashSet->GetMasterHash())
		{
			if (cAICHClients > 0){
				if (!pCurClient->HasLowID())
...
void CPartFile::AICHRecoveryDataAvailable(uint16 nPart)
{
	...
	if (pVerifiedHash == nullptr || !pVerifiedHash->GetHashValid()){
	...
	try {
		CreateHashFromFile(m_hpartfile, PARTSIZE * nPart, length, nullptr, &htOurHash);
	} catch (const CIOFailureException& e) {
	...
	for (uint32 pos = 0; pos < length; pos += EMBLOCKSIZE){
		...
		if ( pVerifiedBlock == nullptr || pOurBlock == nullptr || !pVerifiedBlock->GetHashValid() || !pOurBlock->GetHashValid()){
			wxFAIL;
			continue;
		}
...
void CPartFile::Init()
{
	...
	m_nCompleteSourcesTime = time(nullptr);
...
uint32 CPartFile::GetDlActiveTime() const
{
	...
	if (m_tActivated != 0) {
		nDlActiveTime += time(nullptr) - m_tActivated;
	}
