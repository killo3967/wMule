...
bool CUpDownClient::IsDifferentPartBlock() const // [Tarod 12/22/2002]
{
	...
	{
		Requested_Block_Struct* last_done_block = nullptr;
		Requested_Block_Struct* next_requested_block = nullptr;
...
void CUpDownClient::CreateNextBlockPackage()
{
	try {
		...
		while (!m_BlockRequests_queue.empty()
			...
			CPartFile* srcPartFile = srcfile->IsPartFile() ? static_cast<CPartFile*>(srcfile) : nullptr;
...
void CUpDownClient::FlushSendBlocks()
{
    ...
    if (m_socket) {    //socket may be nullptr...
...
void CUpDownClient::SendHashsetPacket(const CMD4Hash& forfileid)
{
	...
	if ( !file ) {
		from_dq = true;
		if ((file = theApp->downloadqueue->GetFileByID(forfileid)) == nullptr) {
...
{
	if (!m_bCommentDirty || file == nullptr || !ExtProtocolAvailable() || m_byAcceptCommentVer < 1) {
		return;
	}
