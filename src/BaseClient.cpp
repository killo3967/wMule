...
CUpDownClient::CUpDownClient(uint16 in_port, uint32 in_userid, uint32 in_serverip, uint16 in_serverport, CPartFile* in_reqfile, bool ed2kID, bool checkfriend)
{
	...
	if (checkfriend) {
		if ((m_Friend = theApp->friendlist->FindFriend(CMD4Hash(), m_dwUserIP, m_nUserPort)) != nullptr){
...
void CUpDownClient::Init()
{
	...
	m_cShowDR = 0;
	m_reqfile = nullptr;	 // No file required yet
	m_nTransferredUp = 0;
	m_cSendblock = 0;
	m_cAsked = 0;
	msReceivedPrev = 0;
...
void CUpDownClient::Safe_Delete()
{
	...
	Notify_SharedCtrlRemoveClient(ECID(), (CKnownFile*)nullptr);
	Notify_SourceCtrlRemoveSource(ECID(), (CPartFile*)nullptr);
	...
	SetRequestFile( nullptr );

	SetUploadFileID(nullptr);
...
bool CUpDownClient::ProcessHelloTypePacket(const CMemFile& data)
{
	...
	if ((m_Friend = theApp->friendlist->FindFriend(m_UserHash, m_dwUserIP, m_nUserPort)) != nullptr){
...
void CUpDownClient::SetSocket(CClientTCPSocket* socket)
{
	...
	} else if (m_socket != nullptr && socket == nullptr) {
...
void CUpDownClient::SendSharedFilesOfDirectory(const wxString& strReqDir)
{
	...
	if (strReqDir == OP_INCOMPLETE_SHARED_FILES) {
		...
		for (int i = 0; i < iQueuedFiles; i++) {
			...
			if (pFile == nullptr || pFile->GetStatus(true) != PS_READY) {
				continue;
			}
	...
	while (!list.empty()) {
		if (!list.front()->IsLargeFile() || SupportsLargeFiles()) {
			list.front()->CreateOfferedFilePacket(&tempfile, nullptr, this);
		}
		list.pop_front();
	}
...
{
	if (!Kademlia::CKademlia::IsRunning() || Kademlia::CKademlia::GetUDPListener() == nullptr) {
	...
	} else if (Kademlia::CKademlia::GetRoutingZone()->GetContact(wxUINT32_SWAP_ALWAYS(GetConnectIP()), 0, false) != nullptr) {
		errorAlreadyKnown = true;
	}
...
//		DebugLogWarning(_T("UDP Firewallcheck requested with SenderKey == 0 (%s)"), GetClientFullInfo());
	...
	Kademlia::CKademlia::GetUDPListener()->SendPacket(testPacket1, KADEMLIA2_FIREWALLUDP, wxUINT32_SWAP_ALWAYS(GetConnectIP()), remoteInternPort, Kademlia::CKadUDPKey(senderKey, theApp->GetPublicIP(false)), nullptr);
	...
	if (remoteExternPort != 0 && remoteExternPort != remoteInternPort) {
		...
		Kademlia::CKademlia::GetUDPListener()->SendPacket(testPacket2, KADEMLIA2_FIREWALLUDP, wxUINT32_SWAP_ALWAYS(GetConnectIP()), remoteExternPort, Kademlia::CKadUDPKey(senderKey, theApp->GetPublicIP(false)), nullptr);
	}
