...
{
	wxCHECK_RET(owner, wxT("nullptr owner in CGenericClientListCtrl::AddSource"));
...
{
	// A nullptr owner means remove it no matter what.
	wxCHECK_RET(source, wxT("nullptr source in CGenericClientListCtrl::RemoveSource"));
...
void CGenericClientListCtrl::ShowSources( const CKnownFileVector& files )
{
	...
	for (unsigned i = 0; i < files.size(); ++i) {
		...
		if (!std::binary_search(m_knownfiles.begin(), m_knownfiles.end(), files[i])) {
			...
			wxASSERT_MSG(file, wxT("nullptr file in CGenericClientListCtrl::ShowSources"));
