...
int CEncryptedDatagramSocket::DecryptReceivedClient(uint8_t *bufIn, int bufLen, uint8_t **bufOut, uint32_t ip, uint32_t *receiverVerifyKey, uint32_t *senderVerifyKey)
{
	...
	*bufOut = bufIn;
	...
	if (receiverVerifyKey == nullptr || senderVerifyKey == nullptr) {
		wxFAIL;
		return result;
	}
	...
	*senderVerifyKey = 0;
	...
	uint8_t tries;
	if (Kademlia::CKademlia::GetPrefs() == nullptr) {
...
//			DebugLog(_T("Received obfuscated UDP packet from clientIP: %s with wrong key marker bits (kad packet, recvkey key, nodeid bit)"), ipstr(dwIP));
		...
		if (padLen > 0) {
			receivebuffer.RC4Crypt(nullptr, nullptr, padLen);
		}
...
// clientHashOrKadID != nullptr					-> clientHashOrKadID
// clientHashOrKadID == nullptr && kad && receiverVerifyKey != 0	-> receiverVerifyKey
// else								-> ASSERT
int CEncryptedDatagramSocket::EncryptSendClient(uint8_t **buf, int bufLen, const uint8_t *clientHashOrKadID, bool kad, uint32_t receiverVerifyKey, uint32_t senderVerifyKey)
{
	...
	wxASSERT(clientHashOrKadID != nullptr || receiverVerifyKey != 0);
	...
	MD5Sum md5;
	if (kad) {
		if ((clientHashOrKadID == nullptr || CMD4Hash(clientHashOrKadID).IsEmpty()) && receiverVerifyKey != 0) {
			kadRecvKeyUsed = true;
			uint8_t keyData[6];
		...
		else if (clientHashOrKadID != nullptr && !CMD4Hash(clientHashOrKadID).IsEmpty()) {
			uint8_t keyData[18];
...
int CEncryptedDatagramSocket::DecryptReceivedServer(uint8_t* pbyBufIn, int nBufLen, uint8_t **ppbyBufOut, uint32_t dwBaseKey, uint32_t /*dbgIP*/)
{
	...
	*ppbyBufOut = pbyBufIn;
	...
	if (dwValue == MAGICVALUE_UDP_SYNC_SERVER) {
		...
		if (byPadLen > 0) {
			receivebuffer.RC4Crypt(nullptr, nullptr, byPadLen);
		}
