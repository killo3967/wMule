...
bool PlatformSpecific::CreateSparseFile(const CPath& name, uint64_t size)
{
	...
	HANDLE hd = CreateFileW(name.GetRaw().c_str(),
		...
		nullptr,    // security - not inheritable
		CREATE_ALWAYS,
	...
	if (!DeviceIoControl(hd, FSCTL_SET_SPARSE, nullptr, 0, nullptr, 0, &dwReturnedBytes, nullptr)) {
		...
		// zero the data
		if (!DeviceIoControl(hd, FSCTL_SET_ZERO_DATA, (LPVOID) &fzdi, sizeof(fzdi), nullptr, 0, &dwReturnedBytes, nullptr)) {
			AddDebugLogLineC(logPartFile, CFormat(wxT("converting %s to sparse failed (ZERO): %s")) % name % SystemError());
		} else if (!SetFilePointerEx(hd, largo, nullptr, FILE_BEGIN) || !SetEndOfFile(hd)) {
...
static PlatformSpecific::EFSType doGetFilesystemType(const CPath& path)
{
	...
	if (!GetVolumeInformationW(volume, nullptr, 0, nullptr, &maximumComponentLength, &filesystemFlags, filesystemNameBuffer, 128)) {
		return PlatformSpecific::fsOther;
	}
