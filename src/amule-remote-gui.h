			...
			case STATUS_REQ_SENT:
				...
				if ( this->Phase1Done(packet) ) {
					if (this->m_inc_tags) {
						...
						ProcessUpdate(packet, nullptr, m_full_req_tag);
					} else {
...
public:
	...
	T *GetByID(I id)
	{
		...
		return m_items_hash.count(id) ? m_items_hash[id] : nullptr;
	}
...
class CKnownFilesRem : public CRemoteContainer<CKnownFile, uint32, CEC_SharedFile_Tag> {
	CKnownFile * CreateKnownFile(const CEC_SharedFile_Tag *tag, CKnownFile *file = nullptr);
...
public:
	...
	// template
	//
	CKnownFile *CreateItem(const CEC_SharedFile_Tag *) { wxFAIL; return nullptr; }	// unused, required by template
