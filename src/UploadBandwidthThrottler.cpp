 ...
 *              last in the list.
 *
 * @param socket the address to the socket that should be added to the list. If the address is nullptr,
		...
		if (bytesToSpend >= 1) {
			...
			for (uint32 slotCounter = 0; slotCounter < slots; slotCounter++) {
				...
				if (socket != nullptr) {
					...
					AddDebugLogLineN(logGeneral, CFormat( wxT("There was a nullptr socket in the UploadBandwidthThrottler Standard list (trickle)! Prevented usage. Index: %i Size: %i"))
			...
			for (uint32 slotCounter = 0; (slotCounter < slots * 2) && spentBytes < bytesToSpend; slotCounter++) {
				...
				if (socket != nullptr) {
					...
					AddDebugLogLineN(logGeneral, CFormat(wxT("There was a nullptr socket in the UploadBandwidthThrottler Standard list (equal-for-all)! Prevented usage. Index: %i Size: %i"))
