...
void CKnownFile::CreateHashFromHashlist(const ArrayOfCMD4Hash& hashes, CMD4Hash* Output)
{
	...
	CreateHashFromInput(&buffer[0], buffer.size(), Output, nullptr);
}
...
{
	if(m_lastPublishTimeKadNotes > (uint32)time(nullptr)) {
		return false;
	}

	if(!GetFileComment().IsEmpty()) {
		m_lastPublishTimeKadNotes = (uint32)time(nullptr)+KADEMLIAREPUBLISHTIMEN;
		return true;
	}
...
bool CKnownFile::PublishSrc()
{
	...
	if( theApp->IsFirewalled() ) {
		...
		if( buddy ) {
			...
			if( lastBuddyIP != m_lastBuddyIP ) {
				SetLastPublishTimeKadSrc( (uint32)time(nullptr)+KADEMLIAREPUBLISHTIMES, lastBuddyIP );
				return true;
			}
		} else {
			return false;
	...
	if(m_lastPublishTimeKadSrc > (uint32)time(nullptr)) {
		return false;
	}
