...
///
/// Pixmaps from wMule http://www.amule.org
///
...
WxCasFrame::WxCasFrame ( const wxString & title ) :
		wxFrame ( ( wxFrame * ) nullptr, -1, title, wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE )
{
	// Give it an icon
#ifdef __WINDOWS__
...
WxCasFrame::OnFtpUpdateTimer ( wxTimerEvent& WXUNUSED( event ) )
{
	...
	if ( prefs->Read ( WxCasCte::ENABLE_AUTOSTATIMG_KEY,
	                   WxCasCte::DEFAULT_AUTOSTATIMG_ISENABLED )
		 && prefs->Read ( WxCasCte::ENABLE_FTP_UPDATE_KEY,
