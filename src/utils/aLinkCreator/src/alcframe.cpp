...
AlcFrame::AlcFrame (const wxString & title):
    wxFrame ((wxFrame *) nullptr, -1, title)
{
  // Give it an icon
#ifdef __WINDOWS__
  wxIcon icon(wxT("alc"));
  ...
  m_inputUrlListBox = new wxListBox(m_mainPanel, -1, wxDefaultPosition,
                                    wxDefaultSize, 0, nullptr, wxLB_SINGLE | wxLB_NEEDED_SB | wxLB_HSCROLL);
...
AlcFrame::SetFileToHash()
{
	...
	wxString browseroot;
	LPITEMIDLIST pidl;
	HRESULT hr = SHGetSpecialFolderLocation(nullptr, CSIDL_PERSONAL, &pidl);
	if (SUCCEEDED(hr)) {
  ...
  if (!goAhead)
    {
      ...
      ::wxGetApp().GetMainFrame()->m_progressBar = nullptr;
    }
