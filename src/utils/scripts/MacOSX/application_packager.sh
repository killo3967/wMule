...
cp ${SRC_FOLDER}/src/amule wMule.app/Contents/MacOS/
cp ${SRC_FOLDER}/src/webserver/src/amuleweb wMule.app/Contents/MacOS/
cp ${SRC_FOLDER}/src/ed2k aMule.app/Contents/MacOS/
cp ${SRC_FOLDER}/src/amulecmd wMule.app/Contents/MacOS/
cp ${SRC_FOLDER}/platforms/MacOSX/wMule-Xcode/amule.icns wMule.app/Contents/Resources/
...
cp ${SRC_FOLDER}/src/amulegui wMuleGUI.app/Contents/MacOS/
cp ${SRC_FOLDER}/platforms/MacOSX/wMule-Xcode/amule.icns wMuleGUI.app/Contents/Resources/
echo "Done"
echo ""
...
for i in $( otool -L	wMule.app/Contents/MacOS/amule \
						wMule.app/Contents/MacOS/amuleweb \
						aMule.app/Contents/MacOS/ed2k \
						wMule.app/Contents/MacOS/amulecmd \
			| sort -u | grep libwx_ | cut -d " " -f 1 ); do
	cp $i aMule.app/Contents/Frameworks;
done
for i in $( otool -L	wMuleGUI.app/Contents/MacOS/amulegui \
			| sort -u | grep libwx_ | cut -d " " -f 1 ); do
