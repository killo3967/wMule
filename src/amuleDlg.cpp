...
m_serverwnd(nullptr),
m_sharedfileswnd(nullptr),
m_searchwnd(nullptr),
m_chatwnd(nullptr),
m_statisticswnd(nullptr),
m_kademliawnd(nullptr),
m_prefsDialog(nullptr),
m_srv_split_pos(0),
m_imagelist(16,16),
m_tblist(32,32),
m_prefsVisible(false),
m_wndToolbar(nullptr),
...
#ifndef CLIENT_GUI
	if (m_is_safe_state) {
		CPartFileConvertDlg::ShowGUI(nullptr);
	}
#endif
}
...
void CamuleDlg::ShowConnectionState(bool skinChanged)
{
	...
	if (ed2kState != s_ED2KOldState || kadState != s_EKadOldState) {
		...
		if (statusIcon.GetRefData() == nullptr) {
			return;
		}
...
void CamuleDlg::ShowTransferRate()
{
	...
	if( thePrefs::ShowOverhead() )
	{
		buffer = CFormat(_("Up: %.1f%s (%.1f) | Down: %.1f%s (%.1f)"))
				 % (showMBpsUp ? MBpsUp : kBpsUp) % (showMBpsUp ? wxString(_(" MB/s")) : ((kBpsUp > 0) ? wxString(_(" kB/s")) : wxString(wxT("")))) % (theStats::GetUpOverheadRate() / 1024.0)
				 % (showMBpsDown ? MBpsDown : kBpsDown) % (showMBpsDown ? wxString(_(" MB/s")) : ((kBpsDown > 0) ? wxString(_(" kB/s")) : wxString(wxT("")))) % (theStats::GetDownOverheadRate() / 1024.0);
	} else {
		buffer = CFormat(_("Up: %.1f%s | Down: %.1f%s"))
				 % (showMBpsUp ? MBpsUp : kBpsUp) % (showMBpsUp ? wxString(_(" MB/s")) : ((kBpsUp > 0) ? wxString(_(" kB/s")) : wxString(wxT(""))))
				 % (showMBpsDown ? MBpsDown : kBpsDown) % (showMBpsDown ? wxString(_(" MB/s")) : ((kBpsDown > 0) ? wxString(_(" kB/s")) : wxString(wxT(""))));
	}
	...
	wxASSERT((m_wndTaskbarNotifier != nullptr) == thePrefs::UseTrayIcon());
...
void CamuleDlg::Create_Toolbar(bool orientation)
{
	...
	if (current) {
		...
		if (oldorientation != orientation) {
			current->Destroy();
			SetToolBar(nullptr); // Remove old one if present
			m_wndToolbar = nullptr;
		} else {
			current->ClearTools();
		}
