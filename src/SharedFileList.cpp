...
protected:
	...
	CPublishKeyword* FindKeyword(const wxString& rstrKeyword, CKeyWordList::iterator* ppos = nullptr);
};
...
void CSharedFileList::SendListToServer(){
	...
	for ( ; (sorted_it != SortedList.end()) && (count < limit); ++sorted_it ) {
		...
		if (!file->IsLargeFile() || (server && server->SupportsLargeFilesTCP())) {
			file->CreateOfferedFilePacket(&files, server, nullptr);
		}
...
void CSharedFileList::Publish()
{
	...
	unsigned int tNow = time(nullptr);
			...
			if(tNow >= m_lastPublishKadSrc) {
				...
				if(pCurKnownFile) {
					if(pCurKnownFile->PublishSrc()) {
						...
						if(Kademlia::CSearchManager::PrepareLookup(Kademlia::CSearch::STOREFILE, true, kadFileID )==nullptr) {
			...
			if(tNow >= m_lastPublishKadNotes) {
				...
				if(pCurKnownFile) {
					if(pCurKnownFile->PublishNotes()) {
						Kademlia::CUInt128 kadFileID;
						kadFileID.SetValueBE(pCurKnownFile->GetFileHash().GetHash());
						if(Kademlia::CSearchManager::PrepareLookup(Kademlia::CSearch::STORENOTES, true, kadFileID )==nullptr)
