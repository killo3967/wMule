...
CaMuleExternalConnector::CaMuleExternalConnector()
	: m_configFile(nullptr),
	  ...
	  m_locale(nullptr),
	  m_strFullVersion(nullptr),
	  m_strOSDescription(nullptr)
{
...
#else
		Show(prompt + wxT("$ "));
		const char *text = fgets(buffer, buffer_size, stdin);	// == buffer if ok, nullptr if eof
#endif /* HAVE_LIBREADLINE */
		if ( text ) {
...
void CaMuleExternalConnector::ConnectAndRun(const wxString &ProgName, const wxString& ProgVersion)
{
	...
	if (!m_password.IsEmpty()) {
		...
		// Create the socket
		Show(_("\nCreating client...\n"));
		m_ECClient = new CRemoteConnect(nullptr);
		m_ECClient->SetCapabilities(m_ZLIB, true, false);	// ZLIB, UTF8 numbers, notification

		// ConnectToCore is blocking since m_ECClient was initialized with nullptr
...
wxString CaMuleExternalConnector::SetLocale(const wxString& language)
{
	...
	return m_locale == nullptr ? wxString() : m_locale->GetCanonicalName();
}
