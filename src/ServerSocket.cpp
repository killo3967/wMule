	...
	switch (nErrorCode) {
		...
		case wxSOCKET_NOERROR:
			...
			if (cur_server->HasDynIP()) {
				...
				// GetServerByAddress may return nullptr, so we must test!
				...
				if (pServer) {
					...
					AddDebugLogLineN(logServer, wxT("theApp->serverlist->GetServerByAddress() returned nullptr"));
					return;
				}
			}
		...
		switch(opcode) {
			...
			case OP_SEARCHRESULT: {
				...
				CServer* cur_srv = (serverconnect) ?
					serverconnect->GetCurrentServer() : nullptr;
	...
	if (ip) {
		if (theApp->ipfilter->IsFiltered(ip, true)) {
			...
			if ( !m_bNoCrypt && thePrefs::IsServerCryptLayerTCPRequested() && cur_server->GetObfuscationPortTCP() != 0 && cur_server->SupportsObfuscationTCP()){
				...
				SetConnectionEncryption(false, nullptr, true);
			}
