...
{
	MULE_VALIDATE_PARAMS(buffer, wxT("Attempting to write to nullptr buffer."));
...
{
	MULE_VALIDATE_PARAMS(buffer, wxT("Attempting to read from nullptr buffer."));
...
{
	MULE_VALIDATE_PARAMS(puSize, wxT("nullptr pointer argument in ReadBsob"));
...
{
	// We only need to set the the nullptr terminator, since we know that
...
void CFileDataIO::WriteStringCore(const char *s, EUtf8Str eEncode, uint8 SizeLen)
{
	...
	// Only attempt to write non-nullptr strings.
	if (sLength) {
		// No nullptr terminator is written since we explicitly specify the length
		Write(s, sLength);
	}
}
...
{
	MULE_VALIDATE_PARAMS(taglist, wxT("nullptr pointer argument in ReadTagPtrList"));
