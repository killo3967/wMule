...
bool CamulewebApp::GetTemplateDir(const wxString& templateName, wxString& templateDir)
{
	...
	OSStatus status = LSFindApplicationForInfo(
		...
		CFSTR("org.amule.wMule"),
		nullptr, nullptr, &amuleBundleUrl);
	if (status == noErr && amuleBundleUrl) {
		CFBundleRef amuleBundle = CFBundleCreate(nullptr, amuleBundleUrl);
	...
	dir = wxStandardPaths::Get().GetResourcesDir();	// Returns 'wMule' when we use 'amule' elsewhere
