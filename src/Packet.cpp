...
CPacket::~CPacket()
{
	// Never deletes pBuffer when completebuffer is not nullptr
	if (completebuffer) {
		delete [] completebuffer;
	} else if (pBuffer) {
	// On the other hand, if completebuffer is nullptr and pBuffer is not nullptr
		delete [] pBuffer;
	}
...
uint8_t* CPacket::DetachPacket() {
	if (completebuffer) {
		...
		completebuffer = pBuffer = nullptr;
		return result;
	} else{
		if (tempbuffer){
			delete[] tempbuffer;
...
bool CPacket::UnPackPacket(uint32 uMaxDecompressedSize) {
	...
	if (result == Z_OK) {
		wxASSERT( completebuffer == nullptr );
		wxASSERT( pBuffer != nullptr );
