...
CUpDownClient* CClientList::FindClientByIP( uint32 clientip )
{
	...
	return (range.first != range.second) ? range.first->second.GetClient() : nullptr;
}
		...
		switch (cur_client->GetKadState()) {
			...
			case KS_CONNECTED_FWCHECK:
				...
				if (cur_client->GetKadVersion() >= 7) {
					...
					Kademlia::CKademlia::GetUDPListener()->SendNullPacket(KADEMLIA_FIREWALLED_ACK_RES, wxUINT32_SWAP_ALWAYS(cur_client->GetIP()), cur_client->GetKadPort(), 0, nullptr);
				}
...
bool CClientList::SendChatMessage(uint64 client_id, const wxString& message)
{
	...
	if (client) {
		...
		client = new CUpDownClient(PORT_FROM_GUI_ID(client_id),IP_FROM_GUI_ID(client_id),0,0,nullptr, true, true);
		AddClient(client);
	}
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8_t connectOptions)
{
	...
	if (!pNewClient) {
		...
		pNewClient = new CUpDownClient(contact->GetTCPPort(), contact->GetIPAddress(), 0, 0, nullptr, false, true);
...
bool CClientList::IncomingBuddy(Kademlia::CContact* contact, Kademlia::CUInt128* buddyID)
{
	...
	//Add client to the lists to be processed.
	CUpDownClient* pNewClient = new CUpDownClient(contact->GetTCPPort(), contact->GetIPAddress(), 0, 0, nullptr, false, true );
	pNewClient->SetKadPort(contact->GetUDPPort());
...
{
	wxCHECK_RET(torem, wxT("nullptr pointer in RemoveFromKadList"));
...
{
	wxCHECK_RET(toadd, wxT("nullptr pointer in AddToKadList"));
				...
				if (!(pCurClient->GetUploadState() == US_NONE || (pCurClient->GetUploadState() == US_BANNED && !pCurClient->IsBanned()))) {
					AddDebugLogLineN(logProxy,
						CFormat(wxT("Debug: Not deleted client %p with up state: %i "))
							% reinterpret_cast<void*>(pCurClient) % pCurClient->GetUploadState());
				}
				if (!(pCurClient->GetDownloadState() == DS_NONE)) {
					AddDebugLogLineN(logProxy,
						CFormat(wxT("Debug: Not deleted client %p with down state: %i "))
							% reinterpret_cast<void*>(pCurClient) % pCurClient->GetDownloadState());
				}
				if (!(pCurClient->GetChatState() == MS_NONE)) {
					AddDebugLogLineN(logProxy,
						CFormat(wxT("Debug: Not deleted client %p with chat state: %i "))
							% reinterpret_cast<void*>(pCurClient) % pCurClient->GetChatState());
				}
				if (!(pCurClient->GetKadState() == KS_NONE)) {
					AddDebugLogLineN(logProxy,
						CFormat(wxT("Debug: Not deleted client %p with kad state: %i ip: %s"))
							% reinterpret_cast<void*>(pCurClient) % static_cast<int>(pCurClient->GetKadState()) % pCurClient->GetFullIP());
				}
				if (!(pCurClient->GetSocket() == nullptr)) {
					AddDebugLogLineN(logProxy,
						CFormat(wxT("Debug: Not deleted client %p: has socket")) % reinterpret_cast<void*>(pCurClient));
				}
				AddDebugLogLineN(logProxy,
					CFormat(wxT("Debug: Not deleted client %p with kad version: %i"))
						% reinterpret_cast<void*>(pCurClient) % pCurClient->GetKadVersion());
#endif
			}
		}
