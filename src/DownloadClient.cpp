...
{
	// 0.42e
	if (m_socket==nullptr || m_reqfile==nullptr) {
		return;
	}
...
void CUpDownClient::ProcessFileInfo(const CMemFile* data, const CPartFile* file)
{
	// 0.42e
	if (file==nullptr) {
		throw wxString(wxT("ERROR: Wrong file ID (ProcessFileInfo; file==nullptr)"));
	}
	if (m_reqfile==nullptr) {
		throw wxString(wxT("ERROR: Wrong file ID (ProcessFileInfo; m_reqfile==nullptr)"));
	}
	if (file != m_reqfile) {
...
{
	// 0.42e
	wxString strReqFileNull(wxT("ERROR: Wrong file ID (ProcessFileStatus; m_reqfile==nullptr)"));
...
void CUpDownClient::UDPReaskFNF()
{
	...
	if (GetDownloadState() != DS_DOWNLOADING){
		...
		if (!m_socket) {
			if (Disconnected(wxT("UDPReaskFNF m_socket=nullptr"))) {
				Safe_Delete();
			}
		}
	} else {
...
void CUpDownClient::UDPReaskForDownload()
{
	...
	*/
	...
	if (m_nUDPPort != 0 && !theApp->IsFirewalled() && !IsConnected()) {
		...
		theStats::AddUpOverheadFileRequest(response->GetPacketSize());
		theApp->clientudp->SendPacket(response, GetBuddyIP(), GetBuddyPort(), false, nullptr, true, 0 );
	}
}
...
void CUpDownClient::SetReqFileAICHHash(CAICHHash* val){
	if(m_pReqFileAICHHash != nullptr && m_pReqFileAICHHash != val)
...
void CUpDownClient::ProcessAICHAnswer(const uint8_t* packet, uint32 size)
{
	...
	if (pPartFile != nullptr && request.m_pPartFile == pPartFile && request.m_pClient.GetClient() == this && nPart == request.m_nPart){
...
void CUpDownClient::ProcessAICHFileHash(CMemFile* data, const CPartFile* file){
	...
	if(pPartFile != nullptr && pPartFile == GetRequestFile()){
