...
CFileArea::CFileArea()
	: m_buffer(nullptr), m_mmap_buffer(nullptr), m_length(0), m_next(nullptr), m_file(nullptr), m_error(false)
{
...
{
	if (m_buffer != nullptr && m_mmap_buffer == nullptr)
	{
		delete[] m_buffer;
		m_buffer = nullptr;
	}
...
void CFileArea::ReadAt(CFileAutoClose& file, uint64 offset, size_t count)
{
	...
	if (gs_pageSize > 0 && offEnd < 0x100000000ull) {
		...
		void *p = mmap(nullptr, m_length, PROT_READ, MAP_SHARED, file.fd(), offStart);
		if (p != MAP_FAILED) {
			m_file = &file;
...
void CFileArea::StartWriteAt(CFileAutoClose& file, uint64 offset, size_t count)
{
	...
	if (file.GetLength() >= offEnd && gs_pageSize > 0 && offEnd < 0x100000000ull) {
		uint64 offStart = offset & (~((uint64)gs_pageSize-1));
		m_length = offEnd - offStart;
		void *p = mmap(nullptr, m_length, PROT_READ|PROT_WRITE, MAP_SHARED, file.fd(), offStart);
		if (p != MAP_FAILED)
		{
			m_file = &file;
			m_mmap_buffer = (uint8_t*) p;
