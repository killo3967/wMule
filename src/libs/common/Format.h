...
public:
	...
	friend wxString operator+(const wxString& lhs, const CFormat& rhs) {
		return lhs + rhs.GetString();
	}

	friend bool operator==(const wxString& lhs, const CFormat& rhs) {
		return lhs == rhs.GetString();
	}

	friend bool operator==(const CFormat& lhs, const wxString& rhs) {
