...
inline void DoSplitPath(const wxString& strPath, wxString* path, wxString* name)
{
	...
	wxString* pVol = (path ? &vol : nullptr);
	wxString* pExt = (name ? &ext : nullptr);
...
{
	CPath path;
	::DoSplitPath(m_printable, &path.m_printable, nullptr);
	::DoSplitPath(m_filesystem, &path.m_filesystem, nullptr);
...
{
	CPath path;
	::DoSplitPath(m_printable, nullptr, &path.m_printable);
	::DoSplitPath(m_filesystem, nullptr, &path.m_filesystem);
...
{
	wxLongLong free;
	if (::wxGetDiskSpace(path.m_filesystem, nullptr, &free)) {
