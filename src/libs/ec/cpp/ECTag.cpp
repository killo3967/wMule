...
CECTag::CECTag() :
	...
	m_dataLen(0),
	m_tagData(nullptr)	// All access functions check m_dataType, so no need to allocate a dummy buffer.
{
}
...
bool CECTag::WriteTag(CECSocket& socket) const
{
	...
	if (m_dataLen > 0) {
		if (m_tagData != nullptr) {	// This is here only to make sure everything, it should not be nullptr at this point
 ...
 * @return the tag found, or nullptr.
 */
...
CMD4Hash CECTag::GetMD4Data() const
{
	...
	EC_ASSERT(m_tagData != nullptr);

	// Doesn't matter if m_tagData is nullptr in CMD4Hash(),
 ...
 *
 * \return The child tag, or nullptr if index out of range.
 */
