...
static void FormatValue(CFormat& format, const CECTag* tag)
{
	...
	if (tmp_tag) {
		...
		EC_STATTREE_NODE_VALUE_TYPE tmp_valueType = tmp_vt != nullptr ? (EC_STATTREE_NODE_VALUE_TYPE)tmp_vt->GetInt() : EC_VALUE_INTEGER;
	...
	EC_STATTREE_NODE_VALUE_TYPE valueType = vt != nullptr ? (EC_STATTREE_NODE_VALUE_TYPE)vt->GetInt() : EC_VALUE_INTEGER;
	switch (valueType) {
		case EC_VALUE_INTEGER:
