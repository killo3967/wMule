...
class CECTag {
	public:
		...
		bool		AddTag(const CECTag& tag, CValueMap* valuemap = nullptr);
		void		AddTag(ec_tagname_t name, uint64_t data, CValueMap* valuemap = nullptr);
		...
		#ifdef USE_WX_EXTENSIONS
		void		AddTag(ec_tagname_t name, const wxString& data, CValueMap* valuemap = nullptr);
		#endif
		...
		// If tag exists, return its value and store it in target (if target != nullptr)
...
class CECEmptyTag : public CECTag {
	public:
		CECEmptyTag(ec_tagname_t name = 0) : CECTag(name, 0, (const void *) nullptr) {}
};
