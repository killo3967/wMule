...
wxString CSearchList::StartNewSearch(uint32* searchID, SearchType type, CSearchParams& params)
{
	...
	bool supports64bit = type == KadSearch ? true : theApp->serverconnect->GetCurrentServer() != nullptr && (theApp->serverconnect->GetCurrentServer()->GetTCPFlags() & SRV_TCPFLG_LARGEFILES);
