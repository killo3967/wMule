...
CRoutingZone::CRoutingZone()
{
	...
	Init(nullptr, 0, CUInt128((uint32_t)0));
}
...
void CRoutingZone::Init(CRoutingZone *super_zone, int level, const CUInt128& zone_index)
{
	...
	m_nextSmallTimer = time(nullptr) + m_zoneIndex.Get32BitChunk(3);

	// Start this zone.
	StartTimer();

	// If we are initializing the root node, read in our saved contact list.
	if ((m_superZone == nullptr) && (m_filename.Length() > 0)) {
		ReadFile();
	}
}
...
{
	if (m_superZone != nullptr || (m_filename.IsEmpty() && specialNodesdat.IsEmpty())) {
		wxFAIL;
		return;
	}
...
CContact *CRoutingZone::GetContact(uint32_t ip, uint16_t port, bool tcpPort) const throw()
{
	if (IsLeaf()) {
		...
		return (contact != nullptr) ? contact : m_subZones[1]->GetContact(ip, port, tcpPort);
	}
}
...
CContact *CRoutingZone::GetRandomContact(uint32_t maxType, uint32_t minKadVersion) const
{
	if (IsLeaf()) {
		...
		return (contact != nullptr) ? contact : m_subZones[1 - zone]->GetRandomContact(maxType, minKadVersion);
	}
}
...
void CRoutingZone::OnSmallTimer()
{
	...
	if (c != nullptr) {
		...
		if (c->GetVersion() >= 6) {
			...
			DebugSend(Kad2HelloReq, c->GetIPAddress(), c->GetUDPPort());
			CKademlia::GetUDPListener()->SendMyDetails(KADEMLIA2_HELLO_REQ, c->GetIPAddress(), c->GetUDPPort(), c->GetVersion(), 0, nullptr, false);
