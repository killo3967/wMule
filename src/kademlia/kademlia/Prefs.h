...
public:
	...
	bool	HasHadContact() const throw()			{ return m_lastContact ? ((time(nullptr) - m_lastContact) < KADEMLIADISCONNECTDELAY) : false; }
	void	SetLastContact() throw()			{ m_lastContact = time(nullptr); }
	bool	HasLostConnection() const throw()		{ return m_lastContact ? !((time(nullptr) - m_lastContact) < KADEMLIADISCONNECTDELAY) : false; }
