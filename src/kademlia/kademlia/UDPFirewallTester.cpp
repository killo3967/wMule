...
{	// try the next available client for the firewallcheck
	...
	if (!CKademlia::IsRunning() || CKademlia::GetRoutingZone() == nullptr) {
		wxFAIL;
		return;
	}
	...
	while (!m_possibleTestClients.empty()) {
		...
		if (!alreadyRequested && CKademlia::GetRoutingZone()->GetContact(curContact.GetIPAddress(), 0, false) == nullptr) {
