...
CSearch::~CSearch()
{
	...
	if (m_nodeSpecialSearchRequester != nullptr) {
	...
	if (CKademlia::IsRunning() && GetNodeLoad() > 20) {
		switch(GetSearchTypes()) {
			case CSearch::STOREKEYWORD:
				Kademlia::CKademlia::GetIndexed()->AddLoad(GetTarget(), ((uint32_t)(DAY2S(7)*((double)GetNodeLoad()/100.0))+(uint32_t)time(nullptr)));
				break;
		}
	}
...
void CSearch::PrepareToStop() throw()
{
	...
	m_created = time(nullptr) - baseTime + SEC(15);
	m_stopping = true;
}
...
void CSearch::JumpStart()
{
	...
	if ((time_t)(m_lastResponse + SEC(3)) > time(nullptr)) {
		return;
	}
	...
	if (m_requestedMoreNodesContact == nullptr && GetRequestContactCount() == KADEMLIA_FIND_VALUE && m_tried.size() >= 3 * KADEMLIA_FIND_VALUE) {
	...
	switch (m_type) {
			...
			if (from->GetVersion() >= 3) {
				...
				if (file) {
					...
					if (from->GetVersion() >= 6) {
						...
						CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA2_SEARCH_SOURCE_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
				...
				CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA_SEARCH_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
			}
		...
		case KEYWORD: {
			...
			if (from->GetVersion() >= 6) {
				...
				CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA2_SEARCH_KEY_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
				wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
			} else {
				CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA_SEARCH_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
			}
			...
			if (from->GetVersion() >= 3) {
				...
				if (file) {
					...
					if (from->GetVersion() >= 6) {
						...
						CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA2_SEARCH_NOTES_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
						wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
					}
				} else {
					PrepareToStop();
				...
				CKademlia::GetUDPListener()->SendPacket(searchTerms, KADEMLIA_SEARCH_NOTES_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
			}
			m_totalRequestAnswers++;
			break;
		}
		...
		case STOREKEYWORD: {
			...
			while (count && (itListFileID != m_fileIDs.end())) {
				...
				if (from->GetVersion() >= 6) {
					...
					DebugSend(Kad2PublishKeyReq, from->GetIPAddress(), from->GetUDPPort());
					CKademlia::GetUDPListener()->SendPacket(packetdata, KADEMLIA2_PUBLISH_KEY_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
					wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
				} else {
					wxFAIL;
				}
		...
		case STORENOTES: {
			...
			if (file) {
				...
				if (from->GetVersion() >= 6) {
					...
					DebugSend(Kad2PublishNotesReq, from->GetIPAddress(), from->GetUDPPort());
					CKademlia::GetUDPListener()->SendPacket(packetdata, KADEMLIA2_PUBLISH_NOTES_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
					wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
				} else {
					wxFAIL;
				}
		...
		case FINDBUDDY:
		{
			...
			if (from->GetVersion() >= 6) {
				...
				CKademlia::GetUDPListener()->SendPacket(packetdata, KADEMLIA_FINDBUDDY_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
				wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
			}
			m_answers++;
			break;
		...
		case FINDSOURCE:
		{
			...
			if (from->GetVersion() >= 6) {
				...
				CKademlia::GetUDPListener()->SendPacket( packetdata, KADEMLIA_CALLBACK_REQ, from->GetIPAddress(), from->GetUDPPort(), 0, nullptr);
				wxASSERT(from->GetUDPKey() == CKadUDPKey(0));
			}
			m_answers++;
			break;
...
void CSearch::ProcessResultNotes(const CUInt128& answer, TagPtrList *info)
{
	...
	for (TagPtrList::iterator it = info->begin(); it != info->end(); ++it) {
		...
		if (!tag->GetName().Cmp(TAG_SOURCEIP)) {
			...
			entry->AddTag(tag);
			*it = nullptr;	// Prevent actual data being freed
	...
	try {
		...
		if (contact->GetVersion() >= 2) {
			if (contact->GetVersion() >= 6) {
				...
				CKademlia::GetUDPListener()->SendPacket(packetdata, KADEMLIA2_REQ, contact->GetIPAddress(), contact->GetUDPPort(), 0, nullptr);
