...
void CEntry::WriteTagListInc(CFileDataIO* data, uint32_t increaseTagNumber)
{
	...
	wxCHECK_RET(data != nullptr, wxT("data must not be nullptr"));
...
void CKeyEntry::MergeIPsAndFilenames(CKeyEntry* fromEntry)
{
	...
	if (fromEntry == nullptr || fromEntry->m_publishingIPs == nullptr) {
		wxASSERT(fromEntry == nullptr);
		// if called with nullptr, this is a complete new entry and we need to initialize our lists
		if (m_publishingIPs == nullptr) {
		...
		for (PublishingIPList::iterator it = m_publishingIPs->begin(); it != m_publishingIPs->end(); ++it) {
			if (it->m_ip == m_uIP) {
				refresh = true;
				if ((time(nullptr) - it->m_lastPublish) < (KADEMLIAREPUBLISHTIMES - HR2S(1))) {
	...
	if (!refresh) {
		wxASSERT(m_uIP != 0);
		sPublishingIP add = { m_uIP, time(nullptr) };
