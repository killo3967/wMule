...
CIndexed::CIndexed()
{
	...
	m_lastClean = time(nullptr) + (60*30);
...
bool CIndexed::AddKeyword(const CUInt128& keyID, const CUInt128& sourceID, Kademlia::CKeyEntry* entry, uint8_t& load)
{
	...
	if (entry->m_uSize == 0 || entry->GetCommonFileName().IsEmpty() || entry->GetTagCount() == 0 || entry->m_tLifeTime < time(nullptr)) {
		return false;
	}

	KeyHashMap::iterator itKeyHash = m_Keyword_map.find(keyID);
	KeyHash* currKeyHash = nullptr;
	if (itKeyHash == m_Keyword_map.end()) {
		Source* currSource = new Source;
		currSource->sourceID = sourceID;
		entry->MergeIPsAndFilenames(nullptr); // IpTracking init
		...
		if (itSource != currKeyHash->m_Source_map.end()) {
			...
			if (!currSource->entryList.empty()) {
				...
				entry->MergeIPsAndFilenames(oldEntry);	// oldEntry can be nullptr, that's ok and we still need to do this call in this case
				if (oldEntry == nullptr) {
					m_totalIndexKeyword++;
...
bool CIndexed::AddSources(const CUInt128& keyID, const CUInt128& sourceID, Kademlia::CEntry* entry, uint8_t& load)
{
	...
	if( entry->m_uIP == 0 || entry->m_uTCPport == 0 || entry->m_uUDPport == 0 || entry->GetTagCount() == 0 || entry->m_tLifeTime < time(nullptr)) {
		return false;
	}

	SrcHash* currSrcHash = nullptr;
...
bool CIndexed::AddLoad(const CUInt128& keyID, uint32_t timet)
{
	...
	if ((uint32_t)time(nullptr) > timet) {
		return false;
	}
						...
						if (count < 0) {
							...
							if (!oldClient || currName->m_uSize <= OLD_MAX_FILE_SIZE) {
								...
								if (count % 50 == 0) {
									...
									CKademlia::GetUDPListener()->SendPacket(packetdata, KADEMLIA2_SEARCH_RES, ip, port, senderKey, nullptr);
...
bool CIndexed::SendStoreRequest(const CUInt128& keyID)
{
	...
	if (it != m_Load_map.end()) {
		load = it->second;
		if (load->time < (uint32_t)time(nullptr)) {
			m_Load_map.erase(it);
			m_totalIndexLoad--;
			delete load;
			return true;
