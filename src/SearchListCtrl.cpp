...
{
	wxCHECK_RET(src && dst, wxT("nullptr argument in SyncLists"));
...
void CSearchListCtrl::DownloadSelected(int category)
{
	...
	if (category == -1) {
		...
		if (CastByID(IDC_EXTENDEDSEARCHCHECK, nullptr, wxCheckBox)->GetValue()) {
			category = CastByID(ID_AUTOCATASSIGN, nullptr, wxChoice)->GetSelection();
		}
	}
