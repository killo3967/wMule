...
- [ ] Auditar `ClientUDPSocket.cpp`, `CEMSocket.cpp`, `ED2KSocket.cpp`, `KademliaUDPListener.cpp`.
- [ ] Validar longitudes antes de `memcpy`/`Read`.
- [ ] Sustituir buffers estáticos por `std::vector<uint8_t>` o `CMemFile` seguro.
- [ ] Rechazar opcodes desconocidos o tamaños fuera de rango.

#### 1.2 Protección contra SSRF / Reflection en UPnP
- [ ] Revisar `UPnPBase.cpp` extremo a extremo (descubrimiento → mapping → cleanup).
- [ ] Validar `URLBase`, origen, TTL, esquema y puertos antes de aceptar respuestas.
- [ ] Limitar quién puede anunciarse (solo red local), añadir timeouts y máximo de intentos.
- [ ] Registrar intentos sospechosos y exponer estado de mapping en UI/logs.
- [ ] Garantizar fallback claro si UPnP falla (sin dejar al usuario sin conectividad).

#### 1.3 Tests y métricas
- [ ] Crear suite unitaria de payloads válidos/inválidos.
- [ ] Añadir fuzzing ligero (inputs truncados, extendidos, repetidos).
...
- `documentacion/Auditoria_Seguridad_aMule.md` – lista de vulnerabilidades y soluciones sugeridas.
- `documentacion/PLAN_MODERNIZACION_WINDOWS.md` – estado de fases 1-4.
